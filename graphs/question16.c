...
void computeShortestDistance(struct graph *newGraph, int *visited, int *dist, int src) {
	...
	while(!isStackEmpty()){
		...
		struct adjListNode *temp = newGraph->arr[index].head;
		while(temp) {
			dist[temp->data] = findMin(dist[temp->data], temp->cost + dist[index]);
			temp = temp->next;
		}
	}
...
int main() {
	...
	computeShortestDistance(newGraph, visited, dist, src);
